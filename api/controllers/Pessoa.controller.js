...
class PessoaController {
   static async pegaTodasPessoas(req, res){
    ...
    try {
        const todasPessoas = await database.Pessoas.scope('ativo').findAll()
  ...
  static async restauraMatricula(req, res) {
    ...
    try {
        ...
        return res.status(200).json({message: `o id ${matriculaId} foi restaurado`})
    } catch (error) {
        return res.status(500).json(error.message)
    }
  }
